...
*/
ncepNamAlaskaSurfInitialization::ncepNamAlaskaSurfInitialization(ncepNamAlaskaSurfInitialization const& A ) : wxModelInitialization(A)
{
...
* Destructor.
*/
ncepNamAlaskaSurfInitialization::~ncepNamAlaskaSurfInitialization()
{
}
...
*/
ncepNamAlaskaSurfInitialization& ncepNamAlaskaSurfInitialization::operator= (ncepNamAlaskaSurfInitialization const& A)
{
    if(&A != this) {
...
*/
std::vector<std::string> ncepNamAlaskaSurfInitialization::getVariableList()
{
...
int ncepNamAlaskaSurfInitialization::getEndHour()
{
    return 84;
}
...
*/
bool ncepNamAlaskaSurfInitialization::identify( std::string fileName )
{
    ...
    /*
     * Check the global attributes for the following tag:
     * :location = "Proto fmrc:NAM_Alaska_11km"
     */
    /*
    char* location;
    ...
    if( status != NC_NOERR )
        ...
        if( locationString != "Proto fmrc:NAM_Alaska_11km" ) {
            identified = false;
        }
        delete[] location;
    }
...
/**
* Sets the surface grids based on a ncepNamAlaska (surface only!) forecast.
...
*/
void ncepNamAlaskaSurfInitialization::setSurfaceGrids( WindNinjaInputs &input,
