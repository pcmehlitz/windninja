...
// turn provided data set into a dataset with UTM projection
// if no driver name is supplied the source dataset driver is used
...
template <typename T>
void gdalProcessScanLines (GDALDataset* pDS, int bandNo, void (*func)(int,int,T*)) {
    int nBands = pDS->GetRasterCount();
    int nX = pDS->GetRasterXSize();
    int nY = pDS->GetRasterYSize();
...
template void gdalProcessScanLines<double> (GDALDataset* pDS, int bandNo, void (*func)(int,int,double*));
template void gdalProcessScanLines<float> (GDALDataset* pDS, int bandNo, void (*func)(int,int,float*));
template void gdalProcessScanLines<long> (GDALDataset* pDS, int bandNo, void (*func)(int,int,long*));
template void gdalProcessScanLines<unsigned long> (GDALDataset* pDS, int bandNo, void (*func)(int,int,unsigned long*));
template void gdalProcessScanLines<int> (GDALDataset* pDS, int bandNo, void (*func)(int,int,int*));
template void gdalProcessScanLines<unsigned> (GDALDataset* pDS, int bandNo, void (*func)(int,int,unsigned*));
template void gdalProcessScanLines<short> (GDALDataset* pDS, int bandNo, void (*func)(int,int,short*));
template void gdalProcessScanLines<unsigned short> (GDALDataset* pDS, int bandNo, void (*func)(int,int,unsigned short*));
...
void gdalSetSrs (GDALDataset* pDS, int nCols,int nRows, double xllCorner,double yllCorner, double cellSize, const char* pszDstWKT)
{
    double adfGeoTransform[6] = { 
        ...
        yllCorner + nRows * cellSize, 0, -cellSize
    };
...
/**
 * note this is only a VRT, which is owned and hence should not outlive the source dataset
 */
GDALDataset* gdalWarpToVRT (GDALDataset* pSrcDS, const char* csName)
{
    ...
    if (pszSrcWKT && strlen(pszSrcWKT) > 0) {
        ...
        oSRS.SetWellKnownGeogCS( csName);

        char* pszDstWKT = NULL;
        oSRS.exportToWkt( &pszDstWKT );

        return GDALDataset::FromHandle( GDALAutoCreateWarpedVRT(GDALDataset::ToHandle(pSrcDS), pszSrcWKT, pszDstWKT, GRA_NearestNeighbour, 1.0, NULL));

    } else {
        cerr << "input data set has no projection\n";
        return nullptr;
    }
}
